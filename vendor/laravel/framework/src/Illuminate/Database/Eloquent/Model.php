...
abstract class Model implements Arrayable, ArrayAccess, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable
{
    ...
    /**
     * Save the model to the database without logging.
     *
     * @param  array  $options
     * @return bool
     */
    public function saveWithOutLogging(array $options = [])
    {
        $this->mergeAttributesFromClassCasts();
        ...
        // which is typically an auto-increment value managed by the database.
            ...
            $saved = $this->performInsertWithoutLogging($query);
    ...
    /**
     * Perform a model insert operation without logging.
     *
     * @param  \Illuminate\Database\Eloquent\Builder  $query
     * @return bool
     */
    protected function performInsertWithoutLogging(Builder $query)
    {
        if ($this->fireModelEvent('creating') === false) {
            return false;
        }
        ...
        if ($this->getIncrementing()) {
            $this->insertAndSetIdWithoutLogging($query, $attributes);
        }
    ...
    /**
     * Insert the given attributes and set the ID on the model without logging.
     *
     * @param  \Illuminate\Database\Eloquent\Builder  $query
     * @param  array  $attributes
     * @return void
     */
    protected function insertAndSetIdWithoutLogging(Builder $query, $attributes)
    {
        $id = $query->insertGetIdWithoutLogging($attributes, $keyName = $this->getKeyName());
